...
const logger = require("../core/Logger")("storage");
let errCount = 0;
...
*/
    ...
    async function startConn() {
      await sleep(5000);
      await sequelize.sync(syncOpts).then((err) => resolve(self));
    }
  });
};
    ...
    .getRooms(
      {
        ...
        where: {
          "RoomMember.affiliation": affiliation,
          "RoomMember.state": [
            storage.RoomMember.State.Accepted,
            storage.RoomMember.State.Pending,
          ],
        },
      },
      options
...
Storage.prototype.addRoom = function (owner, data, options) {
  ...
  logger.debug(
    "add room " + data.name + " with owner " + JSON.stringify(owner)
  );
...
Storage.prototype.inviteMember = function (data, options) {
  ...
  return room
    ...
    .catch(function () {
      ...
      return room
        .addMember(data.invitee, _.merge(options, opts))
        .then(function () {
...
Storage.prototype.findOrCreateChannel = function (channelname, owner, options) {
  ...
  return storage.Channel.findOrCreate(
    ...
    if (created) {
      ...
      return channel
        .associateUser(channel, owner, [], options)
        .then(function () {
